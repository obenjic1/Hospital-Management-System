	...
	<div class="pagetitle">
		<nav>
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item"><fmt:message key="job.paper"/></li>
	...
	<section class="section">
		<div class="row">
			<div class="col-lg-12">
				<div class="card">
					<div class="card-body">
						<h5 class="card-title"> <fmt:message key="job.tracking"/></h5>
						  ...
						  <thead style="background-color: #dddfe3;">
						    <tr>
						       ...
						       <th scope="col"><fmt:message key="grammage"/></th>
						        <th scope="col"><fmt:message key="openLength"/></th>
						       <th scope="col"><fmt:message key="closeLength"/></th>
						       <th scope="col"><fmt:message key="openWidth"/></th>
						       <th scope="col"><fmt:message key="closeWidth"/></th>
						       <th scope="col"><fmt:message key="volume"/></th>
						        <th scope="col"><fmt:message key="jobColorCombinations"/></th>
						       <th scope="col"><fmt:message key="actions"/></th>
							</tr>
						  </thead>
						  <tbody>
						  <c:forEach var="jobpaper" items="${allJobPaper}" varStatus="loop">
						    <tr class="${loop.index % 2 == 0 ? 'even-row' : 'odd-row'}">
						    <th scope="row">${jobpaper.id}</th>
							   <td>${jobpaper.grammage}</td>
							   <td>${jobpaper.openLength}</td>	
							   <td>${jobpaper.closeLength}</td>
							   <td>${jobpaper.openWidth}</td>
							   <td>${jobpaper.closeWidth}</td>	
							   <td>${jobpaper.volume}</td>	
							   <td>${jobpaper.jobColorCombinations}</td>					   
							   <td>${jobpaper.actions}</td>
							   <td>
							     <a>
