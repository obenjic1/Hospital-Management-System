						...
						<table class="table datatable">
						  <thead style="background-color: #dddfe3;">
						    <tr>
						       ...
						       <th scope="col"><fmt:message key="jobsheet.management"/></th>
						       <th scope="col"><fmt:message key="job.activity.option"/></th>
						       <th scope="col"><fmt:message key="actions"/></th>
							</tr>
						  </thead>
						  <tbody>
						  <c:forEach var="activities" items="${allJobAc}" varStatus="loop">
						    <tr class="${loop.index % 2 == 0 ? 'even-row' : 'odd-row'}">
						    <th scope="row">${activities.id}</th>
							   <td>${activities.job}</td>
							   <td>${jobActivityOption.job}</td>
							   <td>
							     <a>
