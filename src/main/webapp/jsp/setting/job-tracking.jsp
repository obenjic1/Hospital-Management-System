	...
	<div class="pagetitle">
		<nav>
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item"><fmt:message key="job.tracking"/></li>
				...
				<div class="card">
					<div class="card-body">
						...
						<table class="table datatable">
						  ...
						  <tbody>
						  <c:forEach var="tracking" items="${alltracking}" varStatus="loop">
						    <tr class="${loop.index % 2 == 0 ? 'even-row' : 'odd-row'}">
						    <th scope="row">${tracking.id}</th>
							   <td>${tracking.operation}</td>
							   <td>${tracking.job}</td>
							   <td>${tracking.user}</td>
							   <td>${tracking.actions}</td>
							   <td>
							     <a>
