	...
	<div class="pagetitle">
		<nav>
			<ol class="breadcrumb">
				...
				<li class="breadcrumb-item"><fmt:message key="job.operation.option"/></li>
			...
			<div class="col-lg-12">
				<div class="card">
					<div class="card-body">
						...
						<button data-bs-target="#ExtralargeModal" data-bs-toggle="modal" onclick="fillContentModal('')" type="button" class="btn btn-primary" style=" position: relative; left: 94%; width: 77px;">
						...
						<table class="table datatable">
						  <thead style="background-color: #dddfe3;">
						    <tr>
						       ...
						       <th scope="col"><fmt:message key="job.activity.options"/></th>
						        <th scope="col"><fmt:message key="actions"/></th>
							</tr>
						  </thead>
						  <tbody>
						  <c:forEach var="jobopropt" items="${allOprOpt}" varStatus="loop">
						    <tr class="${loop.index % 2 == 0 ? 'even-row' : 'odd-row'}">
						    <th scope="row">${jobopropt.id}</th>
							   <td>${jobopropt.name}</td>
							   <td>${jobopropt.jobActivityOptions}</td>
							   <td>
							     <a>
